"""Check sessions 18–24, 26–29 and 31: links, slides and source consistency."""
...
for session in (*range(18, 25), 26, 27, 28, 29, 31):
...
for path in pages:
    ...
    if path not in pages[:2]:
        anchor = 'ejemplos-locales' if path.name.startswith(('0023-', 'session-23-', '0024-', 'session-24-', '0026-', 'session-26-', '0027-', 'session-27-', '0028-', 'session-28-', '0029-', 'session-29-', '0031-', 'session-31-')) else 'practica-local'
...
slide_count = 0
for session in (22, 23, 24, 26, 27, 28, 31):
